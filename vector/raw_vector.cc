...
template <typename DataType>
int RawVector<DataType>::Add(int docid, struct Field &field) {
  if (ntotal_ >= max_vector_size_) {
    return -1;
  }
  AddToStore((DataType *)field.value.c_str(),
             field.value.size() / sizeof(DataType));

  // add to source
  if (has_source_) {
    int len = field.source.size();
    if (len > 0) {
      memcpy(str_mem_ptr_ + source_mem_pos_[ntotal_], field.source.c_str(),
             len * sizeof(char));
...
template <typename DataType>
int RawVector<DataType>::Update(int docid, struct Field &field) {
  if (vid_mgr_->multi_vids_ || docid >= ntotal_) return -1;
  int vid = docid;

  if (UpdateToStore(vid, (DataType *)field.value.c_str(),
                    field.value.size() / sizeof(DataType))) {
