...
GammaHNSWIndex::GammaHNSWIndex(faiss::Index *quantizer, size_t d,
                               DistanceMetricType metric_type, int M,
                               int efSearch, int efConstruction,
                               const char *docids_bitmap,
                               RawVector<float> *raw_vec)
    : GammaFLATIndex(d, docids_bitmap, raw_vec),
      faiss::IndexHNSW(quantizer, M),
      gamma_hnsw_(M) {
...
bool GammaHNSWIndex::Add(int n, const float *vec) {
  ...
  ntotal = n0 + n;
  AddVertices(n0, n, vec, verbose, gamma_hnsw_.levels.size() == (size_t)ntotal);
...
struct FlatL2Dis : DistanceComputer {
  ...
  explicit FlatL2Dis(size_t d, idx_t nb, const float *xb = nullptr,
                     const float *q = nullptr)
      : d(d), nb(nb), xb(xb), q(q), ndis(0) {}
...
struct FlatIPDis : DistanceComputer {
  ...
  explicit FlatIPDis(size_t d, idx_t nb, const float *xb = nullptr,
                     const float *q = nullptr)
      : d(d), nb(nb), xb(xb), q(q), ndis(0) {}
...
int GammaHNSWIndex::SearchHNSW(int n, const float *x,
...
#pragma omp parallel for schedule(dynamic) num_threads(num_threads)
  for (int i = 0; i < n; i++) {
    ...
    if (reconstruct_from_neighbors &&
      ...
      reconstruct_from_neighbors->compute_distances(k_reorder, idxi, x + i * d,
                                                    simi);
...
int GammaHNSWIndex::Search(VectorQuery &query, GammaSearchCondition *condition,
                           VectorResult &result) {
#ifdef PERFORMANCE_TESTING
  double t0 = utils::getmillisecs();
#endif

  const float *x = reinterpret_cast<const float *>(query.value.c_str());
  int n = query.value.size() / (d * sizeof(float));
...
#ifdef PERFORMANCE_TESTING
  LOG(INFO) << "search " << n << " spend " << utils::getmillisecs() - t0
            << " ms";
#endif  // PERFORMANCE_TESTING
...
int GammaHNSWIndex::Load(const std::vector<std::string> &index_dirs) {
  return 0;
}

GammaHNSWFlatIndex::GammaHNSWFlatIndex(size_t d, DistanceMetricType metric_type,
                                       int nlinks, int efSearch,
                                       int efConstruction,
                                       const char *docids_bitmap,
                                       RawVector<float> *raw_vec)
    : GammaHNSWIndex(new faiss::IndexFlatL2(d), d, metric_type, nlinks,
                     efSearch, efConstruction, docids_bitmap, raw_vec) {
  own_fields = true;
  is_trained = true;
}
