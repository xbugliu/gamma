...
GammaIndexBinaryIVF::GammaIndexBinaryIVF(faiss::IndexBinary *quantizer,
  ...
  rt_invert_index_ptr_ = new realtime::RTInvertIndex(
      ...
      raw_vec_->vid_mgr_, docids_bitmap, 10000, 1280000);
...
int GammaIndexBinaryIVF::Search(VectorQuery &query,
                                GammaSearchCondition *condition,
                                VectorResult &result) {
  const uint8_t *x = reinterpret_cast<const uint8_t *>(query.value.c_str());
  int raw_d = raw_vec_binary_->GetDimension();
  size_t n = query.value.size() / (raw_d * sizeof(uint8_t));

  idx_t *idx = reinterpret_cast<idx_t *>(result.docids);

  const uint8_t *vec_q = x;
