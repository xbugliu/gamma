 ...
 public:
      ...
      case GPU_IVFPQ: {
        ...
        gamma_gpu::GammaIVFPQGPUIndex *gpu_index =
            new gamma_gpu::GammaIVFPQGPUIndex(
                ...
                ivfpq_param->nbits_per_idx, docids_bitmap, raw_vec,
                ivfpq_param->nprobe, counters);
        delete ivfpq_param;
  ...
  static GammaIndex *CreateBinary(RetrievalModel model, size_t dimension,
    ...
    if (dimension % ivfpq_param->nsubvector != 0) {
      ...
      LOG(INFO) << "Dimension [" << raw_d << "] cannot divide by nsubvector ["
                << ivfpq_param->nsubvector << "], adjusted to [" << dimension
                << "]";
    }
    ...
    switch (model) {
      case BINARYIVF: {
        ...
        return (GammaIndex *)new GammaIndexBinaryIVF(
            coarse_quantizer, dimension, ivfpq_param->ncentroids, docids_bitmap,
            raw_vec);
        break;
      }
      default: {
