...
int GammaIVFPQIndex::Delete(int docid) {
  int *vids = raw_vec_->vid_mgr_->docid2vid_[docid];
  if (vids[0] == 0) return 0;
  rt_invert_index_ptr_->Delete(vids + 1, vids[0]);
  return 0;
}
...
int GammaIVFPQIndex::Search(VectorQuery &query, GammaSearchCondition *condition,
                            VectorResult &result) {
  const float *x = reinterpret_cast<const float *>(query.value.c_str());
  int raw_d = raw_vec_->GetDimension();
  size_t n = query.value.size() / (raw_d * sizeof(float));
