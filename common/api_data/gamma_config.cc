...
int Config::Serialize(char **out, int *out_len) {
  ...
  auto config =
      gamma_api::CreateConfig(builder, builder.CreateString(path_),
                              builder.CreateString(log_dir_), max_doc_size_);
...
void Config::Deserialize(const char *data, int len) {
  ...
  max_doc_size_ = config_->max_doc_size();
  path_ = config_->path()->str();
  log_dir_ = config_->log_dir()->str();
}

int Config::MaxDocSize() {
  assert(config_);
  return max_doc_size_;
}

void Config::SetMaxDocSize(int max_doc_size) { max_doc_size_ = max_doc_size; }
