...
int Table::Serialize(char **out, int *out_len) {
  ...
  for (const struct FieldInfo &f : fields_) {
    auto field = gamma_api::CreateFieldInfo(
        ...
        static_cast<gamma_api::DataType>(f.data_type), f.is_index);
  ...
  for (const struct VectorInfo &v : vectors_infos_) {
    auto vectorinfo = gamma_api::CreateVectorInfo(
        ...
        static_cast<gamma_api::DataType>(v.data_type), v.is_index, v.dimension,
...
void Table::Deserialize(const char *data, int len) {
  ...
  vectors_num_ = table_->vectors_num();
...
void Table::SetName(std::string &name) { name_ = name; }

std::vector<struct FieldInfo> &Table::Fields() {
  return fields_;
}

std::vector<struct VectorInfo> &Table::VectorInfos() {
  return vectors_infos_;
}

void Table::AddVectorInfo(struct VectorInfo &vector_info) {
  vectors_infos_.emplace_back(vector_info);
}

void Table::AddField(struct FieldInfo &field) { fields_.emplace_back(field); }

int Table::VectorsNum() {
  return vectors_num_;
}

void Table::SetVectorsNum(int vectors_num) { vectors_num_ = vectors_num; }

int Table::IndexingSize() {
  return indexing_size_;
}

void Table::SetIndexingSize(int indexing_size) {
  indexing_size_ = indexing_size;
}

std::string &Table::RetrievalType() {
  return retrieval_type_;
}

void Table::SetRetrievalType(std::string &retrieval_type) {
  retrieval_type_ = retrieval_type;
}

std::string &Table::RetrievalParam() {
  return retrieval_param_;
}

void Table::SetRetrievalParam(std::string &retrieval_param) {
  retrieval_param_ = retrieval_param;
}

int Table::Read(const std::string &path) {
...
int Table::Write(const std::string &path) {
  char *data;
  int len;
  Serialize(&data, &len);
