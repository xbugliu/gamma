...
int Request::Serialize(char **out, int *out_len) {
  ...
  for (struct TermFilter &term_filter : term_filters_) {
    ...
    bool is_union = term_filter.is_union;
  ...
  auto res = gamma_api::CreateRequest(
      builder, req_num_, topn_, direct_search_type_,
      ...
      static_cast<gamma_api::DistanceMetricType>(metric_type_),
      builder.CreateString(online_log_level_), has_rank_, multi_vector_rank_,
      parallel_based_on_query_, l2_sqrt_);
...
void Request::Deserialize(const char *data, int len) {
  ...
  topn_ = request_->topn();
  direct_search_type_ = request_->direct_search_type();
  ...
  metric_type_ = static_cast<DistanceMetricType>(request_->metric_type());
  online_log_level_ = request_->online_log_level()->str();
  has_rank_ = request_->has_rank();
  multi_vector_rank_ = request_->multi_vector_rank();
  parallel_based_on_query_ = request_->parallel_based_on_query();
...
int Request::DirectSearchType() {
  assert(request_);
  return request_->direct_search_type();
}

void Request::SetDirectSearchType(int direct_search_type) {
  direct_search_type_ = direct_search_type;
}
...
DistanceMetricType Request::MetricType() {
  assert(request_);
  return static_cast<DistanceMetricType>(request_->metric_type());
}

void Request::SetMetricType(DistanceMetricType metric_type) {
  metric_type_ = metric_type;
}

std::string Request::OnlineLogLevel() {
  assert(request_);
  return request_->online_log_level()->str();
}

int Request::HasRank() {
  assert(request_);
  return request_->has_rank();
}

void Request::SetHasRank(int has_rank) { has_rank_ = has_rank; }
...
bool Request::ParallelBasedOnQuery() {
  assert(request_);
  return request_->parallel_based_on_query();
}

void Request::SetParallelBasedOnQuery(bool parallel_based_on_query) {
  parallel_based_on_query_ = parallel_based_on_query;
}
