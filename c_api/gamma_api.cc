...
void *Init(const char *config_str, int len) {
  ...
  int max_doc_size = config.MaxDocSize();
  const std::string &path = config.Path();
  tig_gamma::GammaEngine *engine =
      tig_gamma::GammaEngine::GetInstance(path, max_doc_size);
  if (engine == nullptr) {
...
int DeleteDoc(void *engine, long docid) {
  int ret = static_cast<tig_gamma::GammaEngine *>(engine)->Delete(docid);
  return ret;
}

int GetDocByID(void *engine, long docid, char **doc_str, int *len) {
  tig_gamma::Doc doc;
